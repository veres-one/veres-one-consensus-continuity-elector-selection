...
api.getBlockElectors = async ({
  ...
  // TODO: a cache may be added based on electorPoolDocumentSequence
  const {/*electorPoolDocumentSequence, */maximumElectorCount} = result;
  ...
  const {electors, failureDetectors} = await api._computeElectors({
    ...
    ledgerNode, maximumElectorCount, recoveryMode
  });
...
api._computeElectors = async ({
  blockHeight, electorPoolElectors, latestBlockSummary,
  ledgerNode, maximumElectorCount, recoveryMode
}) => {
  ...
  let electorCount = Math.min(electorPoolElectors.length, maximumElectorCount);
  ...
  while(i--) {
    ...
    if(electorType.includes(ELECTOR_TYPE_GUARANTOR)) {
      ...
      if(mergeEventPeersSet.has(elector.id)) {
        ...
        regularElectors.unshift(elector);
      }
    }
  }
  ...
  // veres-one-validator ensures that an adequate number of failureDetectors are
  // defined in the elector pool
  ...
  while(electors.length < electorCount) {
    ...
    if(failureDetectors.length === failureDetectorCount) {
      // failureDetectors is already populated
      continue;
    }
    if(!failureDetectorCandidate) {
      // veres-one-validator ensures that an adequate number of failureDetectors
      // are defined in the elector pool. This error should never be
      // encountered.
      throw new BedrockError(
        'There are not enough guarantor electors defined in the elector pool.',
        'InvalidStateError', {
          blockHeight,
          electorPoolElectors,
